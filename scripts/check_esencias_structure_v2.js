
const { createClient } = require('@supabase/supabase-js');

const supabaseUrl = 'https://zopuisiwuxcxdozccnip.supabase.co';
// Re-copiada del historial del usuario
const serviceRoleKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InpvcHVpc2l3dXhjeGRvemNjbmlwIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc0NTcxMTQ3OCwiZXhwIjoyMDYxMjg3NDc4fQ.knpcFpOshTEgazVld7_ObrDtcXeYAhz1tqIrb4eJpPo';

const supabase = createClient(supabaseUrl, serviceRoleKey);

async function checkStructure() {
    console.log("Consultando tabla 'esencias' con key corregida...");

    const { data, error } = await supabase
        .from('esencias')
        .select('*')
        .limit(1);

    if (error) {
        console.error("Error al consultar:", JSON.stringify(error, null, 2));
        return;
    }

    if (data && data.length > 0) {
        console.log("Columnas user encontradas:");
        console.log(Object.keys(data[0]));
        console.log("Valor de 'custom_insumos':", data[0].custom_insumos);
    } else {
        console.log("Tabla vacía o sin resultados.");
    }
}

checkStructure();
